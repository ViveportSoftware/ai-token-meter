...
interactive_setup() {
  ...
  # 4. OTEL metrics push
  echo ""
  echo "Metrics can be pushed to a Prometheus/OTLP receiver for Grafana."
  ENABLE_OTEL=false
  if ask_yn "Enable OTEL metrics push (send to Grafana)?" "y"; then
    ENABLE_OTEL=true
  fi
...
write_config() {
  ...
  local otel_block="otel:
  push_endpoint: \"\"
  push_interval: \"30s\""

  if [[ "$enable_otel" == true ]]; then
    otel_block="otel:
  push_endpoint: \"https://atm.htcwowdev.com/api/v1/otlp/v1/metrics\"
  push_interval: \"30s\""
  fi
...
main() {
  ...
  write_config "$UPSTREAM_URL" "$user_id" "$COPILOT" "$ENABLE_OTEL"
  echo ""
